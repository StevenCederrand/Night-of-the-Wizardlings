...
void main() {
    ...
    if (shouldBlend == 0)
    {
        ...
        gl_Position += ((boneMat[bone[1]] * vec4(position, 1.0f)) * weight.y) +((boneMatBlend[bone[1]] * vec4(position, 1.0f)) * weight.y);
        gl_Position += ((boneMat[bone[2]] * vec4(position, 1.0f)) * weight.z) +((boneMatBlend[bone[2]] * vec4(position, 1.0f)) * weight.z);
        gl_Position += ((boneMat[bone[3]] * vec4(position, 1.0f)) * weight.w) +((boneMatBlend[bone[3]] * vec4(position, 1.0f)) * weight.w);
        ...
        //f_position = modelMatrix * gl_Position + f_position;
        //gl_Position = projMatrix * viewMatrix * modelMatrix * gl_Position;
        ...
        f_normal += (vec3((transpose(inverse(mat3(boneMat[bone[1]]))) * normal) * weight.y)) + ( vec3((transpose(inverse(mat3(boneMatBlend[bone[1]]))) * normal) * weight.y));
        f_normal += (vec3((transpose(inverse(mat3(boneMat[bone[2]]))) * normal) * weight.z)) + ( vec3((transpose(inverse(mat3(boneMatBlend[bone[2]]))) * normal) * weight.z));
        f_normal += (vec3((transpose(inverse(mat3(boneMat[bone[3]]))) * normal) * weight.w)) + ( vec3((transpose(inverse(mat3(boneMatBlend[bone[3]]))) * normal) * weight.w));
