...
void DstrGenerator::initPoints(glm::vec2 position)
{
	...
	case BASIC:
		...
		for (int i = 1; i < m_randomPoints.size(); i++)
		{
			...
			{
				offset *= 2.0;	// Increase radius slighly
...
void DstrGenerator::Destroy(DestructibleObject* object, glm::vec2 hitPosition, glm::vec3 hitDirection)
{
	...
	for (int i = 0; i < m_randomPoints.size(); i++)
	{
		...
		if (m_clipped.size() > 0)
		{
			...
			glm::vec3 force = (forceDir * 10) + hitDirection * 1.6;
			if (scale > 0.6)
			...
			float forceRand = rand() % 600;
			forceRand /= 1000;
			force *= (0.8f + forceRand);

			float dirRndX = 2 * (float)rand() / (float)RAND_MAX - 1;
			float dirRndY = 2 * (float)rand() / (float)RAND_MAX - 1;
			float dirRndZ = 2 * (float)rand() / (float)RAND_MAX - 1;
...
const int DstrGenerator::seedRand(int seed)
{
	if (seed == -1)
	{
